...
The `cov-loupe` application provides precise test coverage information for your Ruby application. It reads the `coverage.json` file generated by SimpleCov and exposes that data through multiple subcommands.
...
1. **Detect the failure** – if any MCP tool call fails or the server is unreachable, do not
   retry it or attempt to read `coverage.json` manually. Switch to CLI mode immediately.
