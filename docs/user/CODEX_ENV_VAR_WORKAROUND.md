...
1) Ensure the helper script below exists at `~/.local/bin/codex-cov-loupe` and is executable.
2) Edit `~/.codex/config.toml` to allow env var passthrough (replace `<your-home-dir>` with your actual home directory path):
   ```
   [mcp_servers.cov-loupe]
   command = "<your-home-dir>/.local/bin/codex-cov-loupe"
...
## Helper script (`~/.local/bin/codex-cov-loupe`)
```bash
#!/usr/bin/env bash
set -euo pipefail

# To let Codex pass GEM_HOME/GEM_PATH through to this script, add (or update)
# the following in `~/.codex/config.toml`, then restart Codex:
# [mcp_servers.cov-loupe]
# command = "<your-home-dir>/.local/bin/codex-cov-loupe"
# env_vars = ["GEM_HOME", "GEM_PATH"]
